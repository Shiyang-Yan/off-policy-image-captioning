...
def get_self_critical_reward(batchsize, model, labels, image, gen_result, captions, data, max_tokens):
   # batchsize = gen_result.size(0)# batch_size = sample_size * seq_per_img
    # get greedy decoding baseline
    seq_per_img = 1
    with torch.no_grad():
      outcaps = model.sample_greedy_trigram(labels,image, data.get_vocab())
      res = OrderedDict()
      ...
      res__ = {i: [res[i][0]] for i in range(2 * batchsize)}
     ...
     # bleu_scores = np.array(bleu_scores[3])
      ...
      rewards = scores_out
      return rewards, scores[:batchsize], scores[batchsize:], res__[0], res__[batchsize]
